    ...
    Object.keys(grouped).sort((a,b)=>new Date(b)-new Date(a)).forEach(dk=>{
        ...
        grouped[dk].forEach(item=>{
            ...
            if(item.type==='performance'&&item.branchId){
                ...
                card.innerHTML=`<div>
                    <div class="flex justify-between items-start mb-3"><span class="badge badge-emerald">نظرة عن كثب</span>${delBtn}</div>
                ...
                <div class="mt-auto pt-5 border-t border-white/40 flex justify-between items-center bg-white/40 px-3 py-2 rounded-lg">
                    <span class="text-sm font-bold text-slate-700">الفرع الاستثنائي</span><span class="badge badge-emerald text-xs">الجائزة الاستثنائية</span>
                </div>`;
